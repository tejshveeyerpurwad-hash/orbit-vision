...
export default function Layout({ children }) {
  ...
  return (
    <div className="flex min-h-screen bg-slate-950">
      ...
      {/* Side drawer */}
      <aside className={`fixed top-0 left-0 z-50 flex h-full w-64 flex-col border-r border-white/[0.06] bg-slate-950/95 backdrop-blur-2xl transition-transform duration-300 md:hidden ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      ...
      <div className="flex flex-1 flex-col min-w-0">
        ...
        <header className="sticky top-0 z-30 flex h-11 items-center gap-2 border-b border-white/[0.06] bg-slate-950/85 backdrop-blur-2xl px-2 sm:px-3 shadow-[0_1px_0_rgba(255,255,255,0.03)]">
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <button
              ...
              className={`hidden sm:flex items-center gap-1 rounded-lg border px-2 py-1 text-[8px] font-medium transition-all ${
                presentMode
                  ? 'border-cyan-400/30 bg-cyan-400/10 text-cyan-300'
                  : 'border-white/[0.06] text-slate-500 hover:border-cyan-500/30 hover:text-cyan-400'
              }`}
            >
              <svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            <Link
              to="/cto-report"
              className="inline-flex items-center gap-1 rounded-lg bg-gradient-to-r from-cyan-500 to-blue-600 px-2.5 py-1 text-[8px] font-semibold text-white shadow-lg shadow-cyan-500/20 hover:shadow-xl hover:shadow-cyan-500/30 hover:scale-[1.02] active:scale-[0.98] transition-all"
            >
              <svg className="h-3 w-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        {/* Journey navigation strip — sticky story progress bar */}
